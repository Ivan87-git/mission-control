...
function Column({
  ...
  col: (typeof TASK_STATUS_META)[number];
  tasks: Task[];
  agents: Agent[];
  projects: Project[];
  ...
  return (
    <div className="flex flex-col min-w-[280px]">
      ...
      >
        {tasks.length === 0 ? (
          <div className="flex items-center justify-center h-16 rounded-lg text-xs" style={{ border: "1px dashed var(--border)", color: "var(--text-secondary)", opacity: 0.5 }}>
            No tasks
          </div>
        ) : (
          <div className="space-y-2">
            {sortedGroups.map(([projectId, groupTasks]) => {
              ...
              return (
                <div key={projectId} className="rounded-lg overflow-hidden" style={{ border: "1px solid var(--border)", background: "var(--bg-card)" }}>
                  <div className="flex items-center gap-2 px-2.5 py-2" style={{ background: `${project.color}10`, borderBottom: "1px solid var(--border)" }}>
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full ml-auto flex-shrink-0" style={{ background: "var(--bg-tertiary)", color: "var(--text-secondary)" }}>
                      {groupTasks.length}
                    </span>
                  </div>
...
export default function TaskBoard({
  ...
  return (
    <div className="overflow-x-auto pb-2">
      <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${TASK_STATUS_META.length}, minmax(280px, 1fr))` }}>
        {TASK_STATUS_META.map((col) => (
          <Column key={col.id} col={col} tasks={tasks.filter((task) => task.status === col.id)} agents={agents} projects={projects} onOpenTask={onOpenTask} />
        ))}
      </div>
    </div>
  );
